...
provider "aws" {
    region = "us-east-1" // may need to change this, same region as secrets and ecr
}

// create a secret name git-personal-credentials of type "Other type of secret" on the same region where ecr is located <------------------------- imp
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        FullRepositoryId = "${jsondecode(data.aws_secretsmanager_secret_version.git-credentials.secret_string)["git_user"]}/${var.git_repo}"
...
resource "aws_codebuild_project" "this" {
  ...
  source {
    type            = "CODEPIPELINE"
    buildspec       = "buildspec_testing.yml"
    git_clone_depth = 1
  }
...
/*to test, branch needs to exists bf hand 
clone the app repo and create a testing branch
